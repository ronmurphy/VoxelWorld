...
export class WorkbenchSystem {
    constructor(voxelWorld) {
        ...
        this.recipeBook = {
            ...
            // 🪜 Ladders - Auto-stacking climbing tool
            // 🪜 LADDER CODE COMMENTED OUT - Replaced with grappling hook system
            // ladder_wood: {
            //     name: '🪜 Wooden Ladder',
            //     materials: { wood: 1 },
            //     description: 'Auto-stacking ladder (height = quantity)',
            //     isBasicShape: true,
            //     isLadder: true,  // Special flag for ladder behavior
            //     shapes: [
            //         { type: 'wall', position: { x: 0, y: 0, z: 0 }, size: { x: 1, y: 1, z: 0.1 } }
            //     ]
            // },
            // ladder_leaves: {
            //     name: '🍃 Leaf Ladder',
            //     materials: { leaves: 2 },  // Takes 2 leaves to make 1 ladder
            //     description: 'Auto-stacking ladder from leaves (height = quantity)',
            //     isBasicShape: true,
            //     isLadder: true,  // Special flag for ladder behavior
            //     shapes: [
            //         { type: 'wall', position: { x: 0, y: 0, z: 0 }, size: { x: 1, y: 1, z: 0.1 } }
            ...
            // 🔧 Tool Bench - Unlock tool crafting UI
            tool_bench: {
                name: '🔧 Tool Bench',
    ...
    materialMatches(recipeMaterial, playerMaterial) {
        ...
        if (recipeMaterial === 'leaves') {
            const leafTypes = [
                // Wood-based leaf types (oak_wood-leaves, etc.)
                'oak_wood-leaves', 'pine_wood-leaves', 'birch_wood-leaves', 'palm_wood-leaves', 'dead_wood-leaves',
                // Biome-based leaf types (legacy)
                'forest_leaves', 'mountain_leaves', 'desert_leaves', 'plains_leaves', 'tundra_leaves'
            ];
    ...
    craftItem() {
        ...
        if (this.currentRecipe?.isToolBench) {
            ...
            // Consume materials (3 wood + 3 iron)
        ...
        // Create item data for the naming modal (for regular crafted items)
        const itemData = {
            material: material,
            shape: shape,
            length: length,
