...
class NebulaVoxelApp {
    constructor(container) {
        ...
        this.wasInWater = false; // 🌊 Track water state transitions
        this.keys = {};
        this.selectedSlot = 0;
        ...
        // 🌊 Initialize Physics Engine for movement and swimming
        ...
        // 🔍 Get block data at position (for physics and collision detection)
        this.getBlockAtPosition = (x, y, z) => {
        ...
        this.placeCraftedObject = (x, y, z, itemId) => {
            ...
            // 🔧 WORKAROUND: Pyramids (ConeGeometry) and Walls (ladders) have issues with textured materials in THREE.js r180
            // Force them to use solid color materials only
            if (shapeType === 'pyramid' || shapeType === 'wall') {
                console.log(`Creating solid color material for ${shapeType} (no texture) with color ${color}`);
        ...
        this.spawnStartingBackpack = () => {
            ...
            // 🏝️ CHECK FOR WATER: If backpack is in/over water, create a small cross-shaped island!
            const backpackBlock = this.getBlock(backpackX, groundY - 1, backpackZ);
            const isOverWater = backpackBlock && backpackBlock.type === 'water';

            if (isOverWater) {
                console.log('🌊 Backpack spawned over water - creating cross-shaped island!');

                // Create cross-shaped sand island (5 blocks: center + 4 cardinal directions)
                const islandY = groundY - 1; // Place island at water surface level

                // Center block (under backpack)
                this.addBlock(backpackX, islandY, backpackZ, 'sand', false);

                // North, South, East, West extensions
                this.addBlock(backpackX, islandY, backpackZ + 1, 'sand', false); // North
                this.addBlock(backpackX, islandY, backpackZ - 1, 'sand', false); // South
                this.addBlock(backpackX + 1, islandY, backpackZ, 'sand', false); // East
                this.addBlock(backpackX - 1, islandY, backpackZ, 'sand', false); // West

                console.log(`🏝️ Cross-shaped island created at y=${islandY}`);
            }

            // Place backpack on ground (or on island if created)
        ...
        this.updateTargetHighlight = () => {
            ...
            if (intersects.length > 0) {
                ...
                // 🌊 IGNORE WATER BLOCKS: Don't highlight water for better underwater visibility
                const blockPos = hit.object.position;
                const block = this.getBlock(blockPos.x, blockPos.y, blockPos.z);

                if (block && block.type === 'water') {
                    // Skip water blocks - hide highlight
        ...
        const createBlockMaterial = (blockType, enhancedGraphics = null) => {
            ...
            // Base color (skip solid fill for transparent blocks like water)
            if (blockType.texture !== 'water') {
                ctx.fillStyle = `#${blockType.color.toString(16).padStart(6, '0')}`;
                ctx.fillRect(0, 0, 64, 64);
            } else {
                // 🌊 Water: Start with transparent canvas, add subtle wave patterns only
            ...
            if (blockType.texture === 'grass') {
                ...
                // 🌊 Water texture - light blue tint with subtle wave patterns
                // Fill with very light blue (the material opacity will make it transparent)
                ctx.fillStyle = 'rgba(30, 144, 255, 0.15)'; // Dodger blue, very transparent
                ctx.fillRect(0, 0, 64, 64);

                // Add subtle wave patterns
                ctx.globalAlpha = 0.2;
                ctx.strokeStyle = 'rgba(70, 130, 180, 0.4)'; // Steel blue, semi-transparent
                ctx.lineWidth = 1;
                // Create wave patterns
                for (let i = 0; i < 3; i++) {
                    ctx.beginPath();
                    for (let x = 0; x < 64; x += 4) {
                        const y = 16 + i * 20 + Math.sin(x * 0.1 + i) * 3;
                        if (x === 0) {
                            ctx.moveTo(x, y);
                        } else {
                            ctx.lineTo(x, y);
            ...
            if (blockType.texture === 'glass') {
                ...
                // 🌊 Water material - MeshBasicMaterial for proper transparency (not Lambert!)
                return new THREE.MeshBasicMaterial({
                    color: 0x1E90FF, // Dodger blue
                    transparent: true,
                    opacity: 0.3,
                    side: THREE.DoubleSide, // 🌊 Visible from all angles
                    depthWrite: false  // Prevents rendering issues with overlapping water blocks
                });
            } else if (blockType.texture === 'glow') {
                return new THREE.MeshLambertMaterial({
                    map: texture,
        ...
        // 🔍 Check if a 4x4 area is clear of blocks (terrain and trees) and NOT underwater
            ...
            for (let dx = -halfSize; dx <= halfSize; dx++) {
                for (let dz = -halfSize; dz <= halfSize; dz++) {
                    ...
                    for (let y = 15; y >= 0; y--) {
                        const block = this.getBlock(checkX, y, checkZ);
                        if (block) {
                            // 🌊 WATER CHECK: Reject water surface as spawn point!
                            if (block.type === 'water') {
                                return false; // Don't spawn on/in water
                            }

                            // Found solid surface, check if there are blocks above it (trees/obstacles)
        ...
        // 🔍 Find surface height at a specific position (only solid blocks, not water!)
        this.findSurfaceHeight = (x, z) => {
            for (let y = 15; y >= -5; y--) {
                const block = this.getBlock(x, y, z);
                // 🌊 IMPORTANT: Ignore water blocks - find solid ground only!
                if (block && block.type !== 'water') {
                    return y;
                }
            }
        ...
        this.generateTreesForChunk = (chunkX, chunkZ) => {
            ...
            for (let x = 0; x < this.chunkSize; x++) {
                for (let z = 0; z < this.chunkSize; z++) {
                    ...
                    // 🌊 SKIP OCEAN BIOMES: Don't spawn trees in water!
                    if (biome.name === 'Ocean') {
                    ...
                    if (shouldGenerate) {
                        // 🌍 Find ACTUAL surface (not bedrock, not underground, not water)
                        let surfaceY = -10;
                        for (let y = 15; y >= 1; y--) {  // Start from y=15, stop at y=1 (skip bedrock)
                            const block = this.getBlock(worldX, y, worldZ);
                            // 🌊 Skip water blocks - find solid ground only
                            if (block && block.type !== 'bedrock' && block.type !== 'water') {
                                surfaceY = y + 1; // Place tree 1 block above surface
                                break;
                            }
                        }

                        // Only place tree if we found a valid surface (not bedrock, not water)
        ...
        const animate = (currentTime = 0) => {
            ...
            // ========== 🌊 PHYSICS ENGINE - MOVEMENT & SWIMMING ==========
            ...
            if (Math.abs(dir.x) > 0.001 || Math.abs(dir.z) > 0.001) {
                ...
                this.physics.updateHorizontalMovement(this.player, moveX, moveZ, deltaTime);
            }

            // Apply vertical physics (gravity, jumping, swimming)
            const waterState = this.physics.updateVerticalPhysics(this.player, this.keys, deltaTime);

            // 🌊 Swimming feedback (optional - can add UI indicator later)
            if (waterState.inWater && !this.wasInWater) {
                // Just entered water
                this.wasInWater = true;
            } else if (!waterState.inWater && this.wasInWater) {
                // Just exited water
        ...
        const mousedownHandler = (e) => {
            ...
            if (intersects.length > 0) {
                ...
                if (e.button === 0) { // Left click - harvesting (blocks or crafted objects)
                ...
                } else if (e.button === 2) { // Right click - block placement or grappling hook
                    ...
                    if (selectedBlock && selectedSlot.quantity > 0) {
                        // 🕸️ GRAPPLING HOOK: Check if item is a grappling hook
                        const metadata = this.inventoryMetadata?.[selectedBlock];
                        if (metadata?.isGrapplingHook) {
                            // Teleport player to target block + 2Y
                            ...
                            // Instant teleport (could add smooth animation later)
                            this.player.position.set(targetX, targetY, targetZ);
                            this.player.velocity = 0;  // Reset velocity to prevent fall damage
                        ...
                        // 🎯 Detect crafted items vs regular blocks
                        else if (selectedBlock.startsWith('crafted_')) {
